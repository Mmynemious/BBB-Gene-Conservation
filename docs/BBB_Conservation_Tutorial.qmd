...
Before writing up results, we need to confirm that every gene that entered the CDS alignment pipeline is actually **protein-coding**. If any noncoding genes (lncRNAs, pseudogenes, T-cell receptor segments, etc.) were in the master list and somehow extracted a sequence, the paper's framing — "we measured conservation of BBB coding sequences" — would be wrong. This step audits every Ensembl ID in the master list against the `gene_biotype` annotation embedded in the FASTA headers and checks whether any flagged genes appear in the BBB CDS files.
...
ggplot(df_long, aes(x = PctIdentity, fill = Gene_Set)) +
  ...
  labs(x = "% nucleotide identity", y = "Density",
       ...
       fill = "Gene set",
       caption = "Purple = region where BBB and Liver distributions overlap (red + blue at 50% transparency)") +
  theme_minimal() +
